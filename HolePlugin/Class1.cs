...
namespace HolePlugin
{
    ...
    public class AddHole : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            FamilySymbol familySimbol = new FilteredElementCollector(arDoc)
                .OfClass(typeof(FamilySymbol))
                .OfCategory(BuiltInCategory.OST_GenericModel)
                .OfType<FamilySymbol>()
                .Where(x => x.FamilyName.Equals("Отверстие"))
                .FirstOrDefault();
            if (familySimbol == null)
            {
            ...
            foreach(Duct d in ducts)
            {
                ...
                foreach(ReferenceWithContext refer in intersections)
                {
                    ...
                    FamilyInstance hole = arDoc.Create.NewFamilyInstance(pointHole, familySimbol, wall, level, StructuralType.NonStructural);
